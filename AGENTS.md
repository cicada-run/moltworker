...
This is a Cloudflare Worker that runs [OpenClaw](https://github.com/cicada-run/openclaw) in a Cloudflare Sandbox container. It provides:
...
1. `openclaw.json.template` is copied to `~/.openclaw/openclaw.json` (internal path)
...
| `OPENCLAW_GATEWAY_TOKEN` | `--token` flag | Mapped from `OPENCLAW_GATEWAY_TOKEN` |
